...
/**
 * BeAPI stopped sleeping event. Contains the logic
 * for translating Minecraft event data to BeAPI
 * wrapped data.
 */
export class StoppedSleeping extends AbstractEvent {
  ...
  // Predefined in AbstractEvent.
  @setProto('StoppedSleeping')
  public readonly name = 'StoppedSleeping'
  ...
  protected __logic(data: PlayerTagEvent): void {
    ...
    if (data.tag !== 'wake') return
    // Set is sleeping to false.
    data.player.isSleeping(false)
